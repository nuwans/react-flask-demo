...
# copy assests folder  to  static folder
gulp.task('copy-assets', ->
    gulp.src('assets/**')
      .pipe(gulp.dest('static'))
      .pipe($.size())
)
#copy index  folder to templates  folder
...
###gulp.task "webpack-dev-server", ['css'], (callback) ->
  ...
  devServer.listen 8080, "0.0.0.0", (err) ->
    ...
    gutil.log "[webpack-dev-server]", "http://localhost:8050"
    callback()
  return
###  
...
gulp.task 'build', ['webpack:build', 'copy-assets','copy-index','css'], ->
  gutil.log('Build completed run  python app.py')
 return 

gulp.task 'watch', ['css', 'copy-assets', 'copy-index','webpack:build-dev'], ->
  gulp.watch(['src/styles/**'], ['css'])
  gulp.watch(['assets/**'], ['copy-assets'])
  gutil.log('press CTRL+C  and then run  python app.py')
return
